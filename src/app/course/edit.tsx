...
import { useCourse, useUpdateCourse, useDeleteCourse } from '../../hooks/queries';
import { calculateCourseGrade, validateCourseScores } from '../../lib/calculations';
import { validateCourse, sanitizeString } from '../../lib/validation';
...
export default function EditCourseScreen() {
  ...
  useEffect(() => {
    if (course) {
      setFormData({
        ...
        ia_score: course.ia_score,
        ia_max: course.ia_max,
        ue_score: course.ue_score,
        ue_max: course.ue_max,
      });
    }
  }, [course]);
  ...
  const handleSave = () => {
    ...
    updateCourseMutation.mutate(
      { courseId: courseId || '', data: sanitizedData },
      {
        onSuccess: () => {
          router.back();
        },
          ...
          onPress: () => {
            if (course) {
              deleteCourseMutation.mutate(
                { courseId: courseId || '', semesterId: course.semester_id },
                {
                  onSuccess: () => {
                    router.back();
                  },
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text variant="h3">Course Not Found</Text>
          <View style={{ width: 44 }} />
        </View>
  ...
  return (
    ...
    >
      <View style={styles.header}>
        ...
        <Text variant="h3">Edit Course</Text>
